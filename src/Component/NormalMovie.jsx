...
import { MyContext } from "../Contextapi/ContextApi"; // Import your context
import { IMAGE_BASE_URL } from "../helper/MovieBaseUrl";

import Banner from "../Banner/Banner";

const Movie = () => {
  const { movieUrl, updateBanner,Genre } = useContext(MyContext);
  const fetchMovies = useFetchMovies();

  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["movies", movieUrl ,Genre],
    queryFn: fetchMovies,
  });
    ...
    <div className="w-full ">
      ...
      <div className="w-[90%] min-h-[20vh] flex flex-wrap my-2 gap-2 ">
        ...
        {data?.results?.map((movie) => (
          <div className="w-[30%] sm:w-[19%] h-[70px] sm:h-[100px] hover:w-[21%] cursor-pointer transition-all duration-300"

            onClick={() =>
              BannerImageUpdate(movie)
            }
            key={movie.id}
          >
            <img
